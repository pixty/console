...
type (
	...
	Camera struct {
		Id        int64
		Name      string // display name (unique per org)
		OrgId     int64
	...
	PartTx interface {
		...
		// ==== Cams ====
		InsertCamera(cam *Camera) (int64, error)
		GetCameraById(camId int64) (*Camera, error)
		UpdateCamera(cam *Camera) error
		DeleteCamera(camId int64) error
		...
		DeleteFaces(fcIds []int64) error

		// ==== Pictures ====
		FindZeroRefPics(limit int) ([]*Picture, error)
		DeletePics(pics []*Picture) error
		...
		// ==== Profiles ====
		CheckProfileInOrgWithCam(prId int64, camId int64) (bool, error)
	...
	PersonsQuery struct {
		...
		CamId int64
		// The maximum allowed max time.
		MaxLastSeenAt common.Timestamp
		// The minimum allowed created time
		MinCreatedAt *common.Timestamp
		// The max allowed created time
		MaxCreatedAt *common.Timestamp
		// The number of faces associated with the person
		MinFacesCount int
...
func (q *PersonsQuery) String() string {
	return fmt.Sprintf("{CamId=%d, PersonsIds=%v, MaxLastSeenAt=%d, Limit=%d}", q.CamId, q.PersonIds, q.MaxLastSeenAt, q.Limit)
}
